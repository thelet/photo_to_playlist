...
# Import Spotify functions from pipeline
import sys
src_dir = Path(__file__).parent.parent.parent
if str(src_dir) not in sys.path:
    sys.path.insert(0, str(src_dir))

from pipeline import connect_to_spotify, find_songs_on_spotify, create_spotify_playlist
...
def render_spotify_loading_section():
    ...
    try:
        ...
        if not st.session_state.get("spotify_authenticated", False):
            ...
            # Perform authentication (credentials loaded from environment)
            ...
            if not client:
                ...
                from playlist.spotify_client import get_spotify_credentials
